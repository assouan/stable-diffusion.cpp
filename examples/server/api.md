...
- `vae_tiling`
- `cache`
- `model_options.minimax_h3_attention_sparsity`
- `cancel_queued`
- `cancel_generating`
...
Downloads a completed video result when the server was started with
`--output-dir`. The same file remains in that directory; this endpoint is only
a convenient HTTP view of the persisted result.

Typical status codes:

- `200 OK`
- `404 Not Found`
- `409 Conflict` while the job is not completed
- `410 Gone` after the job metadata expires
...
- `vid_gen` adds `end_image`, `control_frames`, `high_noise_sample_params`, `video_frames`, `fps`, `moe_boundary`, `vace_strength`, and optional model-specific runtime options
...
Model-specific runtime fields:

| Field | Type |
| --- | --- |
| `model_options` | `object` |
| `model_options.minimax_h3_attention_sparsity` | `number` in `[0, 1)` |

`minimax_h3_attention_sparsity` is accepted only when the loaded diffusion
model advertises support for it in `features_by_mode.vid_gen.model_options`.
The value applies only to the submitted job and is restored afterward. Use
`0` for dense attention; the MiniMax-H3 Turbo-SLA profile typically uses
`0.85` together with its matching SLA LoRA.

### MiniMax-H3 generation artifacts

When the server has a persistent `--output-dir`, MiniMax-H3 jobs can save or
reuse intermediate safetensors through the top-level `artifacts` object.
Conditioning artifacts support text-only, I2VA, FL2VA, and reference media.

| Field | Type | Effect |
| --- | --- | --- |
| `artifacts.save_conditioning` | `boolean` | Save the positive multimodal conditioning after the text encoder |
| `artifacts.conditioning_only` | `boolean` | Stop after saving the conditioning, without diffusion sampling or VAE decoding |
| `artifacts.save_latent` | `boolean` | Save the packed audio/video latent after diffusion and before VAE decoding |
| `artifacts.latent_only` | `boolean` | Stop after saving the sampled latent, without final audio/video VAE decoding or media encoding |
| `artifacts.conditioning_input` | `string` | Reuse a saved conditioning and skip the text encoder |
| `artifacts.latent_input` | `string` | Reuse a saved packed latent and skip both the text encoder and diffusion sampling |

For example, run only the encoder and save its conditioning:

```json
{
  "prompt": "a singer performing in a sunlit forest",
  "width": 1344,
  "height": 768,
  "video_frames": 124,
  "fps": 24,
...
Saved files are generated by the server under
`<output-dir>/artifacts/` and are reported as paths relative to
`<output-dir>` in `result.artifacts`. To reuse one, pass only its base file
name, not `artifacts/` or another path:

```json
{
  "artifacts": {
    "conditioning_input": "job_01HTXYZVID.conditioning.safetensors",
    "save_latent": true,
    "latent_only": true
  }
}
...
- inputs must name an existing `.safetensors` regular file directly inside
  `<output-dir>/artifacts/`; parent paths are rejected
- `conditioning_input` cannot be combined with `save_conditioning`
- `conditioning_only` requires `save_conditioning` and cannot be combined with
  another artifact operation
- `latent_only` requires `save_latent`; it may reuse `conditioning_input`, but
  cannot load a latent or save conditioning
- `latent_input` cannot be combined with any other artifact option
- latent metadata must match the requested width, height, normalized frame
  count, FPS, and the loaded MiniMax-H3 latent layout
- artifact jobs require MiniMax-H3 with CFG 1 and no hires pass
- a conditioning generated with media contains Qwen's multimodal output; reuse
  it with the same prompt, media, dimensions, and ordering so the VAE-side
  reference latents can be reconstructed without running Qwen again
- text-only conditioning can be reused at another output resolution, but it
  still represents the exact prompt that originally produced it
- the server does not hash the prompt or media stored outside the artifact;
  matching them on reuse is the caller's responsibility
- a latent already fixes the sampled content and geometry; reusing it is
  intended for VAE or output-container tests
...
- animated WebP uses lossless encoding when `output_compression` is `100`
- `avi` means MJPG AVI
- `webm` requires the server to be built with WebM support; otherwise the request returns `400`

### Result Payload

Completed jobs return one encoded container, not a list of per-frame images.
...
- `result.artifacts.conditioning`, when present, identifies the saved
  conditioning relative to `--output-dir`
- `result.artifacts.latent`, when present, identifies the saved packed latent
  relative to `--output-dir`
- without `--output-dir`, `result.b64_json` contains the whole encoded container as base64
- with `--output-dir`, `result.file_name` identifies the file persisted in that directory and
  `result.download_url` provides an optional HTTP download endpoint; `b64_json` is omitted
...
Example completed job when the server was started with `--output-dir /data/videos`:
...
```json
{
  ...
  "result": {
    ...
    "fps": 16,
    "frame_count": 33,
    "file_name": "job_01HTXYZVID.webm",
    "download_url": "/sdcpp/v1/jobs/job_01HTXYZVID/result",
    "artifacts": {
      "conditioning": "artifacts/job_01HTXYZVID.conditioning.safetensors",
      "latent": "artifacts/job_01HTXYZVID.latent.safetensors"
    }
  },
  "error": null
}
...
The file is already present as `/data/videos/job_01HTXYZVID.webm`. To copy it
through HTTP instead:

```bash
curl -fL http://127.0.0.1:1234/sdcpp/v1/jobs/job_01HTXYZVID/result -o video.webm
```

The persisted file is not deleted when the in-memory job metadata expires.
